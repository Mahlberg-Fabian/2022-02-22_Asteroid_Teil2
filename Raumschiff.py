...
class Kaefer(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.images = []
        self.images.append(pygame.image.load(os.path.join(Settings.image_path, "invider2.jpg")).convert_alpha())                # Image laden | convert.alpha() -> Alphakanal soll erhalten bleiben / hintergrund soll unsichtbar gemacht werden
#        self.images.append(pygame.image.load(os.path.join(Settings.image_path, "spritesheet0.jpg")).convert_alpha())            # Image laden
...
class Apfel(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        r = random.randint(0, 3)                                                                             # Zufallszahl erzeugen von 0 - 4
        self.image = pygame.image.load(os.path.join(Settings.image_path, f"Asteroid{r}.jpg")).convert_alpha()   # Zufallszahl wird in "{r}" eingesetzt
        self.rect = self.image.get_rect()
...
class Game(object):
    ...
    def __init__(self):
        ...
        self.time_spawn_groester_asteroid = 3000                                       # größter Asteroid spawnt alle 3 Sekunden
